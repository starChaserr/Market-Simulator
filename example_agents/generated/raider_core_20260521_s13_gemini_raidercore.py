...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            # Hold or quote smaller when edge is weak
            bid_edge = (fair - target_bid) / mid
            ask_edge = (target_ask - fair) / mid
            
            # Minimum edge required before we scale down or hold
            base_min_edge = 0.00015 # 1.5 bps
            
            if regime == "shock":
                base_min_edge = 0.0005
                b_size *= 0.3
                s_size *= 0.3
            elif regime == "volatile":
                base_min_edge = 0.0003
                b_size *= 0.6
                s_size *= 0.6
                
            if bid_edge < base_min_edge:
                if inventory > 0:
                    b_size = 0.0
                else:
                    b_size *= 0.5
            
            if ask_edge < base_min_edge:
                if inventory < 0:
                    s_size = 0.0
                else:
                    s_size *= 0.5
