...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float, last_fill_time: float):
        ...
        if adverse > 0.65:
            ...
            # Check if equity is stagnant OR no fills for 10s in calm market
            pnl_diff = 0.0
            is_stagnant = False
            if len(self.pnl_history) == self.pnl_history.maxlen:
                pnl_diff = self.pnl_history[-1] - self.pnl_history[0]
                if abs(pnl_diff) < 20.0: is_stagnant = True
            
            # Short-circuit for short sessions or zero-fill starts
            if regime == "calm" and (now - last_fill_time > 5.0):
                is_stagnant = True
...
def trade_loop(args: argparse.Namespace) -> int:
    logging.info("Deploying Autonomous RaiderCore v6.1 (Enhanced Survivability) as %s", args.user)
    ...
    while True:
        ...
        # ── Extreme Dislocation Hold ──
        disloc = abs(fund / mid - 1.0)
        if disloc > 0.02: # 2% dislocation is extreme
            if last_bid > 0 or last_ask > 0:
                cancel_open_orders(args.url, args.user)
                last_bid = last_ask = 0.0
            logging.warning("HOLD: Extreme dislocation (%.2f%%). Pausing quotes.", disloc * 100)
            time.sleep(0.5)
            continue
        ...
        # ── EMERGENCY CANCEL ──
        # If quote is toxic or we are in shock and price moved, cancel immediately ignoring throttle.
        if toxic_quote or (regime == "shock" and price_moved):
            cancel_open_orders(args.url, args.user)
            last_bid = last_ask = 0.0
            last_quote_at = now
            logging.warning("SAFETY: Emergency cancel. Toxic: %s, ShockMove: %s", toxic_quote, (regime == "shock" and price_moved))

        if (price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            cancel_open_orders(args.url, args.user)
            
            # Size adjusted by controller mult and regime
            regime_size_mult = {"shock": 0.2, "volatile": 0.6, "calm": 1.1}.get(regime, 0.9)
