...
def quote_is_stale(state: dict[str, Any], signals: dict[str, float | str]) -> bool:
    age = state.get("quote_age_seconds")
    if age is None:
        return False
    try:
        quote_age = float(age)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            if quote_is_stale(state, signals):
                cancel_open_orders(args.url, args.user)
                last_bid = 0.0
                last_ask = 0.0
                time.sleep(args.interval)
