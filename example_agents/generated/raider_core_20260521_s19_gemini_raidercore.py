...
class ParametricController:
    ...
    def __init__(self):
        ...
        self.target_spread_bps = 3.5 # Start tighter for immediate engagement
        self.skew_intensity = 4.0
        self.order_size_mult = 1.0
        
        # Performance history
        self.pnl_history = deque(maxlen=15) # Window for faster response (22.5s)
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        now = time.time()
        if now - self.last_adjustment_at < 1.5: return # Tuning cycle (1.5s)
        ...
        if adverse > 0.65:
            step = (adverse - 0.65) * 12.0
            self.target_spread_bps += step
        elif adverse < 0.35:
            # Faster tightening if low adverse selection and no fills
            if len(self.pnl_history) >= 3 and fill_rate < 0.1:
                # If no fills for ~4.5s, start drifting tighter
                self.target_spread_bps = max(1.2, self.target_spread_bps - 0.4)
        ...
        self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 0.85)
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        def tick_round(p):
            return round(round(p / args.tick) * args.tick, 4)

        target_bid = tick_round(min(fair - width/2 - skew, best_ask - args.tick))
        target_ask = tick_round(max(fair + width/2 - skew, best_bid + args.tick))

        # ── Opportunistic Tightening ──
        # If we have zero position and our target is within reach of the market, join the book
        # to ensure we don't stay completely idle in short sessions.
        if abs(inventory) < args.max_pos * 0.05:
             if target_bid < best_bid and (best_bid - target_bid) / mid * 10000.0 < 2.0:
                 target_bid = best_bid
             if target_ask > best_ask and (target_ask - best_ask) / mid * 10000.0 < 2.0:
                 target_ask = best_ask
        ...
        # "Fairness" Guardrail: Respect the market's quoting speed but be active in calm
        throttle_gap = {"shock": 2.5, "volatile": 1.0, "calm": 0.3}.get(regime, 0.6)
