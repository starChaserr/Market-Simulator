...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            # Weak edge protection: Holding, cancelling, or quoting smaller
            edge_is_weak = adverse > 0.75 or fill_rate > 4.0
            if edge_is_weak and abs(inventory) < args.max_pos * 0.3:
                # Hold: do not quote if edge is weak and inventory is manageable
                b_size = 0.0
                s_size = 0.0
            else:
                if regime == "shock":
                    order_size *= 0.25  # Quote much smaller in shocks
                elif edge_is_weak:
                    order_size *= 0.5   # Quote smaller if edge is weak but we need to offload
