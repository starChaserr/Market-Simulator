...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        now = time.time()
        if now - self.last_adjustment_at < 1.2: return # Faster tuning cycle
        ...
        if adverse > 0.65:
            step = (adverse - 0.65) * 12.0 # More aggressive step
            ...
            if abs(pnl_diff) < 15.0:
                # Tighten more proactively, especially in short sessions
                tighten_step = 0.5 if abs(pnl_diff) < 5.0 else 0.3
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if controller.target_spread_bps < market_spread_bps * 1.4 or adverse < 0.35:
            # Asymmetric joining based on inventory
            if inventory < args.max_pos * 0.4:
