...
def quote_stale_after(
    ...
    if adverse > 0.75:
        seconds *= clamp(1.0 - (adverse - 0.75) * 0.80, 0.55, 1.0)
    if churn_pressure > 0.0:
        seconds *= 1.0 + churn_pressure * 0.75
    return clamp(seconds, args.interval * 4.0, 90.0)
...
def open_order_side_counts(api_url: str, user: str) -> dict[str, int] | None:
    orders = call_api(api_url, user_path("/orders?limit=1000", user))
    if orders is None:
        return None
    counts: dict[str, int] = {}
    ...
    try:
        while True:
            ...
            if now < sell_pause_until and inventory <= target_inv + pause_band:
                sell_size = 0.0
            if now - last_order_check_at >= max(args.interval, 1.0):
                live_side_counts = open_order_side_counts(args.url, args.user)
                last_order_check_at = now
                if live_side_counts is not None:
                    buy_open = live_side_counts.get("buy", 0)
                    sell_open = live_side_counts.get("sell", 0)
                    if buy_open <= 0:
                        last_bid = 0.0
                    elif buy_open > 1:
            ...
            stale_quote = last_quote_at > 0.0 and now - last_quote_at >= quote_stale_after(
                args,
                signals,
                adverse,
                churn_pressure,
            ...
            refresh_bid = buy_size >= 1.0 and (
                ...
                or (quote_ready and (bid_price_moved or regime != last_regime or refresh_for_risk or stale_quote))
            )
            refresh_ask = sell_size >= 1.0 and (
                ((first_quote or quote_ready) and (ask_missing or ask_unsafe))
                or (quote_ready and (ask_price_moved or regime != last_regime or refresh_for_risk or stale_quote))
            )
