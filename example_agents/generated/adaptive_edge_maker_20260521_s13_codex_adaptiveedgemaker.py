...
def conviction_adjusted_target(
    target_inventory: float,
    signals: dict[str, float | str],
    flow: float,
) -> float:
    if abs(target_inventory) < 1.0:
        return target_inventory

    side = 1.0 if target_inventory > 0.0 else -1.0
    regime = str(signals["regime"])
    dislocation = float(signals["dislocation"])
    trend = float(signals["trend"])
    short_momentum = float(signals["short_momentum"])
    ...
    if abs(flow) > 0.38 and flow * side < 0.0:
        opposing += 1
    if abs(trend) > 0.0012 and trend * side < 0.0:
        opposing += 1
    if abs(short_momentum) > 0.0012 and short_momentum * side < 0.0:
        opposing += 1
    if abs(dislocation) > 0.0012 and dislocation * side < 0.0:
        opposing += 1
    ...
    if regime in {"volatile", "shock"} and opposing > 0:
        scale *= 0.80
    return target_inventory * clamp(scale, 0.25, 1.0)


def sweep_side_allowed(side: str, signals: dict[str, float | str], flow: float) -> bool:
    side_direction = 1.0 if side == "buy" else -1.0
    regime = str(signals["regime"])
    dislocation = float(signals["dislocation"])
    trend = float(signals["trend"])
    short_momentum = float(signals["short_momentum"])
    flow_barrier = 0.42 if regime in {"trend", "volatile", "shock"} else 0.58

    if flow * side_direction < -flow_barrier:
        return False
    if regime in {"trend", "volatile", "shock"}:
        if trend * side_direction < -0.0012:
            return False
        if short_momentum * side_direction < -0.0016 and abs(dislocation) < 0.003:
            return False
    if (
        abs(dislocation) > 0.0012
        and dislocation * side_direction < 0.0
        and abs(flow) > 0.32
    ):
        return False
    return True
...
def maybe_sweep_edge(
    ...
    if (
        blocked_side != "buy"
        and sweep_side_allowed("buy", signals, flow)
        and buy_edge > threshold
        and inventory < max_pos
        and exposure_ratio <= 0.45
    ):
    ...
    if (
        blocked_side != "sell"
        and sweep_side_allowed("sell", signals, flow)
        and sell_edge > threshold
        and inventory > -max_pos
        and exposure_ratio >= -0.45
    ):
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            target_inv = conviction_adjusted_target(target_inv, signals, flow)
            if loss_pressure > 0.0:
            ...
            if can_sweep and time.time() - last_sweep_at >= args.sweep_cooldown:
                if maybe_sweep_edge(args.url, args.user, state, account, fair, size_args, signals, max_pos, flow, blocked_side):
