...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float, last_fill_time: float):
        ...
        if adverse > 0.65:
            ...
            pnl_full = len(self.pnl_history) == self.pnl_history.maxlen
            pnl_diff = self.pnl_history[-1] - self.pnl_history[0] if len(self.pnl_history) > 1 else 0.0
            
            # Tighten if stagnant or no fills for a while
            no_fill_duration = now - last_fill_time
            if (pnl_full and abs(pnl_diff) < 20.0) or (no_fill_duration > 5.0):
                tighten_step = 0.4 if no_fill_duration > 10.0 else 0.2
        ...
        # But if we have 0 fills, we might need to be narrower than 90% of market
        mkt_floor_mult = 0.9 if (now - last_fill_time < 20.0) else 0.7
        self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * mkt_floor_mult)
        ...
        self.target_spread_bps = clamp(self.target_spread_bps, 1.0, 80.0)
        ...
        logging.info("AUTOTUNE: Spread %.1f bps (Mkt %.1f) | Skew %.1f | SizeMult %.2f | NoFill %.1fs", 
                     self.target_spread_bps, market_spread_bps, self.skew_intensity, self.order_size_mult, now - last_fill_time)
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    tracker.last_fill_time = time.time()
    ...
    while True:
        ...
        vol = float(state.get("volatility", 0.0))
        vol_floor = 0.5 if regime in {"shock", "volatile"} else 0.25
        width = max(width, mid * vol * vol_floor)
        ...
        now = time.time()
        no_fill_duration = now - tracker.last_fill_time
        tight_enough = controller.target_spread_bps < market_spread_bps * (2.5 if no_fill_duration > 10.0 else 1.5)
        if tight_enough:
