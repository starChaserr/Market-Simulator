...
def quote_age_limit(
    ...
    if adverse > 0.75:
        age *= clamp(1.0 - (adverse - 0.75) * 1.2, 0.55, 1.0)
    pressure = max(churn_pressure, loss_pressure)
    if pressure > 0.0:
        age *= clamp(1.0 - pressure * 0.50, 0.45, 1.0)
    return clamp(age, max(args.interval * 4.0, 2.0), 60.0)


def market_data_is_unsafe(state: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if quote_age is None:
        return False
    return float(quote_age) > max(5.0, args.interval * 12.0)
...
def book_quality_guarded_sizes(
    buy_size: float,
    sell_size: float,
    state: dict[str, Any],
    args: argparse.Namespace,
    ...
    if buy_size < 1.0 and sell_size < 1.0:
        return buy_size, sell_size
    ...
    live_size = max(buy_size, sell_size, 1.0)
    shallow_side_depth = min(max(bid_volume, 0.0), max(ask_volume, 0.0))
    depth_ratio = shallow_side_depth / max(live_size, 1.0)
    wide_book = spread_ratio > max(args.tick * 8.0 / mid, 0.004, (stress + volatility) * 1.35)
    weak_book = shallow_side_depth <= 0.0 or depth_ratio < 1.25
    thin_book = depth_ratio < 2.50

    risk_base = max(max_pos, 1.0)
    target_gap = inventory - target_inventory
    rebalance_band = risk_base * (0.08 if regime in {"volatile", "shock"} else 0.05)
    buy_rebalances = target_gap < -rebalance_band
    sell_rebalances = target_gap > rebalance_band

    if weak_book or wide_book:
        if buy_rebalances:
            buy_size *= 0.35
        else:
            buy_size = 0.0
        ...
        scale = clamp(depth_ratio / 2.50, 0.35, 0.85)
        if not buy_rebalances:
            buy_size *= scale
        if not sell_rebalances:
            sell_size *= scale
    ...
    try:
        while True:
            ...
            if state.get("mid_price") is None or state.get("best_bid") is None or state.get("best_ask") is None:
                ...
                continue
            if market_data_is_unsafe(state, args):
                if last_bid > 0.0 or last_ask > 0.0:
                    cancel_open_orders(args.url, args.user)
                    last_bid = 0.0
                    last_ask = 0.0
            ...
            buy_size, sell_size = book_quality_guarded_sizes(
                buy_size,
                sell_size,
                state,
                args,
            ...
            quote_stale = last_quote_at > 0.0 and now - last_quote_at >= quote_age_limit(
                args,
                signals,
                adverse,
                churn_pressure,
            ...
            refresh_for_risk = inventory_changed or adverse > 0.80 or quote_stale
