...
def current_microprice(state: dict[str, Any]) -> float:
    ...
    if total <= 0:
        return mid
    raw_micro = (best_ask * bid_vol + best_bid * ask_vol) / total
    imbalance = abs(bid_vol - ask_vol) / total
    micro_weight = clamp(0.78 - imbalance * 0.38, 0.40, 0.78)
    return raw_micro * micro_weight + mid * (1.0 - micro_weight)


def market_data_stale(state: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return False
    return age > max(args.interval * 6.0, 2.5)
    ...
    try:
        while True:
            ...
            if state.get("mid_price") is None or state.get("best_bid") is None or state.get("best_ask") is None:
                ...
                continue
            if market_data_stale(state, args):
                cancel_open_orders(args.url, args.user)
                last_bid = 0.0
                last_ask = 0.0
