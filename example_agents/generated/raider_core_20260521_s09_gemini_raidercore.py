...
class FillTracker:
    ...
    def adverse_score(self) -> float:
        ...
        raw_score = max(buys, n - buys) / n
        # Dampen score if we have very few fills to avoid overreacting
        if n < 5:
            return raw_score * (n / 5.0)
        return raw_score
...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        ...
        self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 0.9, 2.5)
        ...
        self.target_spread_bps = clamp(self.target_spread_bps, 2.5, 80.0)
...
def get_imbalance(state: dict[str, Any]) -> float:
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        imb = get_imbalance(state)
        if regime == "calm":
            fair = fund * 0.2 + micro * 0.4 + mid * 0.4
        else:
            fair = fund * 0.5 + micro * 0.2 + mid * 0.3
        
        # Imbalance adjustment: Shift fair price based on imbalance (up to 15% of spread)
        fair += imb * (best_ask - best_bid) * 0.15

        # ── Dynamic Sizing & Width ──────────────────────────────────────
        # Base width from self-tuned controller
        width = mid * (controller.target_spread_bps / 10000.0)
        
        # "Fair" protection: broaden if volatility is high or spread is too narrow
        vol = float(state.get("volatility", 0.0))
        width = max(width, mid * vol * 0.5)
        
        # Guardrail: If market spread is very narrow, be cautious
        if market_spread_bps < 1.5:
            width *= 1.3
        ...
        if regime in ("calm", "drift") and controller.target_spread_bps < market_spread_bps * 1.5:
        ...
        if int(now) % 5 == 0 and now - last_quote_at > 0.5:
            logging.info("HEARTBEAT: Mid %.2f | Fair %.2f | Imb %.2f | Regime %s | Inv %.1f",
                         mid, fair, imb, regime, inventory)

        # "Fairness" Guardrail: Respect the market's quoting speed
        throttle_gap = {"shock": 3.0, "volatile": 1.5, "calm": 0.5}.get(regime, 0.8)
        
        # Dynamic move threshold: tighter in calm, looser in shock
        move_threshold_mult = {"shock": 0.4, "volatile": 0.3, "calm": 0.15}.get(regime, 0.2)
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            regime_size_mult = {"shock": 0.3, "volatile": 0.6, "calm": 1.0, "drift": 0.8}.get(regime, 1.0)
            order_size = args.order_size * controller.order_size_mult * regime_size_mult
            
            # Fee guard: if market is super narrow, reduce size further
            if market_spread_bps < 1.5:
                order_size *= 0.7
