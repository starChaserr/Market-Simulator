    ...
    try:
        while True:
            ...
            if not state or not account:
                data_miss_count += 1
                if data_miss_count >= 3 and (last_bid > 0.0 or last_ask > 0.0):
            ...
            mid = float(state["mid_price"])
            best_bid_value = float(state["best_bid"])
            best_ask_value = float(state["best_ask"])
            if mid <= 0.0 or best_bid_value <= 0.0 or best_ask_value <= 0.0 or best_bid_value >= best_ask_value:
                if last_bid > 0.0 or last_ask > 0.0:
                    cancel_open_orders(args.url, args.user)
                    last_bid = 0.0
                    last_ask = 0.0
