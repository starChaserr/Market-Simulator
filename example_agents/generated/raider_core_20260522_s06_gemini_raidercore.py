...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        ...
        pnl_diff = self.pnl_history[-1] - self.pnl_history[0] if len(self.pnl_history) > 1 else 0.0
        
        # 1. Proportional Spread Tuning
        # If losing money, widen immediately
        if pnl_diff < -20.0:
            self.target_spread_bps += 5.0
            logging.warning("LOSS BRAKE: Widening spread to %.1f bps", self.target_spread_bps)
        # If adverse selection is high (one side > 75%), widen
        elif adverse > 0.75:
            step = (adverse - 0.75) * 15.0 
            self.target_spread_bps += step
        # If balanced (one side < 62%) and enough history, consider tightening
        elif adverse < 0.62 and len(self.pnl_history) == self.pnl_history.maxlen:
            # Check if equity is stagnant or growing
            if pnl_diff > 5.0: # Winning! Can we be more aggressive?
                self.target_spread_bps = max(1.2, self.target_spread_bps - 0.3)
            elif abs(pnl_diff) < 2.0: # Stagnant
                self.target_spread_bps = max(1.2, self.target_spread_bps - 0.6)

        # 2. Fill Rate Awareness
        if fill_rate > 4.0: # Being hit way too often
            self.target_spread_bps += 2.0
            self.order_size_mult = max(0.1, self.order_size_mult * 0.8)
        ...
        if self.target_spread_bps > market_spread_bps * 3.0 and adverse < 0.55:
        ...
        # 4. Size Tuning
        if adverse > 0.80 or fill_rate > 3.0:
            self.order_size_mult = max(0.1, self.order_size_mult - 0.2)
        elif adverse < 0.65 and fill_rate < 0.5:
            self.order_size_mult = min(2.5, self.order_size_mult + 0.1)

        self.last_adjustment_at = now
        logging.info("AUTOTUNE: Spread %.1f bps (Mkt %.1f) | Skew %.1f | SizeMult %.2f | PnLDelta %.1f", 
                     self.target_spread_bps, market_spread_bps, self.skew_intensity, self.order_size_mult, pnl_diff)
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        # Fair competition: Blend signals (Trust microprice more to avoid being picked off)
        if regime == "calm":
            fair = fund * 0.1 + micro * 0.5 + mid * 0.4
        elif regime == "drift":
            fair = fund * 0.2 + micro * 0.5 + mid * 0.3
        else: # volatile or shock
            fair = fund * 0.1 + micro * 0.7 + mid * 0.2
        ...
        # ── Circuit Breaker ──
        session_pnl = float(account.get("equity", 0)) - args.starting_cash
        if session_pnl < -300.0:
            logging.error("CIRCUIT BREAKER: Session PnL %.2f below limit. Standing down.", session_pnl)
            cancel_open_orders(args.url, args.user)
            time.sleep(10.0)
            continue

        # ── Opportunistic Tightening ──
        # Join market only if balanced and not losing significantly
        if controller.target_spread_bps < market_spread_bps * 1.4 and adverse < 0.65:
            # Asymmetric joining based on inventory
            if inventory < args.max_pos * 0.2:
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            # Defensive scaling for market regimes
            if regime == "shock":
                order_size *= 0.2
            ...
            # Risk Mitigation: If adverse is high, stop quoting the side that adds to risk
            if adverse > 0.85:
                if inventory > 20: b_size = 0.0 # Don't buy more if already long
                if inventory < -20: s_size = 0.0 # Don't sell more if already short
