...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-quote-age", type=float, default=45.0)
...
def price_tick(state: dict[str, Any], args: argparse.Namespace) -> float:
    tick = max(args.tick, 0.01)
    if state.get("mode") == "upstox_paper":
        try:
            configured_tick = float(state.get("tick", tick))
        except (TypeError, ValueError):
            configured_tick = tick
        mid = max(float(state.get("mid_price") or 0.0), 0.01)
        if 0 < configured_tick <= max(1.0, mid * 0.01):
            tick = max(tick, configured_tick)
    return tick
...
def quote_width(state: dict[str, Any], args: argparse.Namespace) -> float:
    ...
    maker_fee = abs(float(state.get("fees", {}).get("maker_fee_rate", 0.0)))
...
def market_is_actionable(state: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if spread > max(mid * 0.012, tick * 12.0):
        return False
    bids = state.get("order_book", {}).get("bids", [])
    asks = state.get("order_book", {}).get("asks", [])
...
def side_permissions(
    ...
    pressure_gate = max(0.00012, tick / max(mid, 0.01))
    if imbalance > 0.62 and pressure > pressure_gate and inventory <= 0:
        allow_sell = False
    elif imbalance < -0.62 and pressure < -pressure_gate and inventory >= 0:
        allow_buy = False
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            bid_changed = desired_bid and abs(bid - last_bid) > tick
            ask_changed = desired_ask and abs(ask - last_ask) > tick
            quote_stale = last_quote_at > 0 and time.time() - last_quote_at >= args.max_quote_age
            should_requote = bid_changed or ask_changed or side_set_changed or quote_stale
            if should_requote:
                cancel_open_orders(args.url, args.user)
