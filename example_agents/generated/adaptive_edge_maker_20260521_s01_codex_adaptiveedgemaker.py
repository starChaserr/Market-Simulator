...
def min_quote_gap(
    ...
    if order_ratio > 0.85:
        gap *= 1.85
    return clamp(gap, args.interval, 14.0)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            can_sweep = (churn_pressure < 0.45 or needs_rebalance) and loss_pressure < 0.72 and order_ratio < sweep_budget_limit
            ...
            quote_ready = now - last_quote_at >= min_quote_gap(
                args, signals, inventory, max_pos, adverse, order_count, churn_pressure
            )
