...
def opportunity_threshold(
    ...
    return threshold * gate_multiplier
...
def opportunity_guarded_sizes(
    ...
    regime = str(signals["regime"])
    pressure = max(loss_pressure, churn_pressure)
    threshold = opportunity_threshold(signals, loss_pressure, churn_pressure, gate_multiplier)
    ...
    if exposure_ratio < 0.06:
        if (
            pressure <= 0.05
            and regime not in {"volatile", "shock"}
            and abs(flow) < (0.40 if regime == "trend" else 0.52)
        ):
            starter_scale = 0.20 if regime == "trend" else 0.26
            return buy_size * starter_scale, sell_size * starter_scale
        return 0.0, 0.0
...
def book_risk_pressure(state: dict[str, Any], reference_size: float, args: argparse.Namespace) -> float:
    mid = max(float(state.get("mid_price", 0.0) or 0.0), 0.01)
    ...
    combined_depth = bid_vol + ask_vol
    reference_depth = max(reference_size, 1.0)
    displayed_spread = max(float(state.get("spread", 0.0) or 0.0), best_ask - best_bid, args.tick)
    spread_ratio = displayed_spread / mid

    shallow_side = clamp((reference_depth * 2.0 - min(bid_vol, ask_vol)) / (reference_depth * 2.0), 0.0, 1.0)
    thin_book = clamp((reference_depth * 5.0 - combined_depth) / (reference_depth * 5.0), 0.0, 1.0)
    one_sided = 1.0
    if combined_depth > 0.0:
        depth_imbalance = abs(bid_vol - ask_vol) / combined_depth
        one_sided = clamp((depth_imbalance - 0.82) / 0.18, 0.0, 1.0)
    wide_spread = clamp((spread_ratio - 0.0015) / 0.0060, 0.0, 1.0)
    return clamp(max(shallow_side, thin_book * 0.75, one_sided * 0.65, wide_spread), 0.0, 1.0)


def weak_flat_liquidity_ok(
    signals: dict[str, float | str],
    flow: float,
    inventory: float,
    max_pos: float,
    ...
    if regime in {"volatile", "shock"}:
        return False
    if adverse > 0.65 or churn_pressure > 0.05 or loss_pressure > 0.05 or book_pressure > 0.55:
        return False
    if abs(flow) > (0.40 if regime == "trend" else 0.52):
        return False
    return abs(inventory) <= max(max_pos * 0.06, 1.0)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            quote_opportunity = opportunity_score(state, signals, flow)
            quote_gate = opportunity_threshold(
                signals,
                loss_pressure,
                churn_pressure,
            ...
            if abs(inventory) / max(max_pos, 1.0) < 0.06 and quote_opportunity < quote_gate:
                target_inv *= clamp(quote_opportunity / max(quote_gate, 1e-9), 0.0, 0.35)
            if loss_pressure > 0.0:
            ...
            book_pressure = book_risk_pressure(state, trading_size, args)
            if book_pressure > 0.0:
                width *= 1.0 + book_pressure * 0.85
                trading_size *= clamp(1.0 - book_pressure * 0.55, 0.35, 1.0)
            if weak_flat_liquidity_ok(signals, flow, inventory, max_pos, adverse, churn_pressure, loss_pressure, book_pressure):
                starter_width = mid * passive_edge_threshold(args, state, signals, adverse, order_count, churn_pressure) * 2.25
                width = max(width, starter_width)
            ...
            can_sweep = (
                ...
                and (book_pressure < 0.70 or needs_rebalance)
            )
            ...
            quote_ready = now - last_quote_at >= min_quote_gap(
                ...
                order_count,
                churn_pressure,
                gap_multiplier=float(control["gap_mult"]) * (1.0 + book_pressure * 1.20),
            )
