...
def market_data_tradeable(state: dict[str, Any], args: argparse.Namespace) -> bool:
    try:
        mid = float(state["mid_price"])
        best_bid = float(state["best_bid"])
        best_ask = float(state["best_ask"])
        reported_spread = float(state.get("spread", best_ask - best_bid) or 0.0)
    except (TypeError, ValueError):
        return False

    if mid <= 0.0 or best_bid <= 0.0 or best_ask <= 0.0 or best_bid >= best_ask:
        return False

    book_spread = best_ask - best_bid
    effective_spread = max(book_spread, reported_spread)
    if effective_spread <= 0.0:
        return False
    if mid < best_bid - args.tick or mid > best_ask + args.tick:
        return False
    return effective_spread / mid <= 0.06
...
def quote_age_limit(
    ...
    if adverse > 0.75:
        limit *= clamp(1.0 - (adverse - 0.75) * 1.2, 0.55, 1.0)
    pressure = max(churn_pressure, loss_pressure)
    if pressure > 0.0:
        limit *= clamp(1.0 - pressure * 0.55, 0.60, 1.0)
    return max(4.0, limit)
    ...
    try:
        while True:
            ...
            if state.get("mid_price") is None or state.get("best_bid") is None or state.get("best_ask") is None:
                ...
                continue
            if not market_data_tradeable(state, args):
            ...
            refresh_bid = buy_size >= 1.0 and (
                ...
                or (
                    ...
                    and (
                        ...
                        or refresh_for_risk
                        or (quote_stale and last_bid > 0.0)
                    )
                )
            )
            refresh_ask = sell_size >= 1.0 and (
                ...
                or (
                    ...
                    and (
                        ...
                        or regime != last_regime
                        or refresh_for_risk
                        or (quote_stale and last_ask > 0.0)
                    )
                )
            )
