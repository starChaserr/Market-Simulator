...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        now = time.time()
        if now - self.last_adjustment_at < 1.0: return # Slightly faster tuning (1.0s)
        ...
        if adverse > 0.85:
            # Extreme adverse selection: widen significantly and instantly
            self.target_spread_bps = min(120.0, self.target_spread_bps * 1.5 + 10.0)
            logging.warning("ADVERSE BRAKE: Widening spread to %.1f bps", self.target_spread_bps)
        elif adverse > 0.65:
            step = (adverse - 0.65) * 15.0 # Increased sensitivity
            ...
            if pnl_diff < 5.0: # Stagnant or losing
                tighten_step = 0.4 if pnl_diff < 0 else 0.2
                self.target_spread_bps = max(1.2, self.target_spread_bps - tighten_step)

        # Market Awareness: Stay at least 1.1x market spread if being hit
        if adverse > 0.5:
        ...
        # Elastic Recovery: More conservative snap back
        if self.target_spread_bps > market_spread_bps * 3.0 and adverse < 0.25:
            self.target_spread_bps = self.target_spread_bps * 0.8 + market_spread_bps * 0.2
            logging.info("ELASTIC RECOVERY: Snapping spread to %.1f bps", self.target_spread_bps)

        self.target_spread_bps = clamp(self.target_spread_bps, 1.2, 120.0)

        # 2. Skew Tuning
        inventory = abs(float(account.get("inventory", 0)))
        if inventory > 400: # Near max_pos (600)
            self.skew_intensity = min(20.0, self.skew_intensity + 1.5)
        elif inventory > 200:
            self.skew_intensity = min(12.0, self.skew_intensity + 0.5)
        elif inventory < 50:
            self.skew_intensity = max(1.5, self.skew_intensity - 0.5)

        # 3. Size Tuning
        if adverse > 0.85:
            self.order_size_mult = max(0.05, self.order_size_mult * 0.5) # Aggressive size cut
        elif adverse > 0.70 or fill_rate > 4.0:
            self.order_size_mult = max(0.15, self.order_size_mult - 0.2)
        elif adverse < 0.40 and fill_rate < 0.5:
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if controller.target_spread_bps < market_spread_bps * 1.5 and adverse < 0.4:
        ...
        # Dynamic throttle: wait longer if we just got filled (anti-sweep)
        ...
        # 1. IMMEDIATE CANCELLATION for safety
        if toxic_quote:
            cancel_open_orders(args.url, args.user)
            last_bid = last_ask = 0.0
            logging.warning("TOXIC QUOTE: Immediate cancellation triggered")

        # 2. STRATEGIC RE-QUOTING
        if (price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            if not toxic_quote: # If not already cancelled
                cancel_open_orders(args.url, args.user)
            ...
            # Stop quoting on the side that increases risk if adverse is high
            if adverse > 0.8:
                if inventory > 0: b_size = 0.0
                if inventory < 0: s_size = 0.0
                logging.warning("RISK OVERLAY: Disabling one side due to adverse selection")
