...
def market_data_stale(state: dict[str, Any], interval: float) -> bool:
    quote_age = state.get("quote_age_seconds")
    if quote_age is None:
        return False
    return float(quote_age) > max(5.0, interval * 8.0)
...
def liquidity_pressure(
    ...
    mid = max(float(state["mid_price"]), 0.01)
    spread_ratio = max(float(state.get("spread", 0.0)), 0.0) / mid
    regime = str(signals["regime"])
    spread_anchor = {
        "calm": 0.0012,
        "mixed": 0.0018,
    ...
    spread_pressure = clamp((spread_ratio - spread_anchor) / max(spread_anchor, 1e-9), 0.0, 1.0)

    bids = state.get("order_book", {}).get("bids", [])
    asks = state.get("order_book", {}).get("asks", [])
    bid_vol = top_volume(bids)
    ask_vol = top_volume(asks)
    thinner_side = min(bid_vol, ask_vol)
    depth_need = max(order_size * 2.5, 1.0)
    depth_pressure = clamp((depth_need - thinner_side) / depth_need, 0.0, 1.0)

    total = bid_vol + ask_vol
    imbalance_pressure = 0.0
    if total > 0.0:
        imbalance_pressure = clamp((abs(bid_vol - ask_vol) / total - 0.82) / 0.18, 0.0, 1.0)

    return max(spread_pressure, depth_pressure, imbalance_pressure * 0.55)


def open_quote_prices(api_url: str, user: str) -> tuple[float, float] | None:
    ...
    for order in orders.get("orders", []):
        ...
        side = order.get("side")
        if side == "buy":
            best_buy = max(best_buy, float(price))
        elif side == "sell":
            best_sell = min(best_sell or float(price), float(price))
    ...
    try:
        while True:
            ...
            if state.get("mid_price") is None or state.get("best_bid") is None or state.get("best_ask") is None:
                ...
                continue
            if market_data_stale(state, args.interval):
                cancel_open_orders(args.url, args.user)
                last_bid = 0.0
                last_ask = 0.0
                time.sleep(args.interval)
            ...
            book_pressure = liquidity_pressure(state, signals, trading_size)
            if book_pressure > 0.0:
                width *= 1.0 + book_pressure * 0.55
                trading_size *= clamp(1.0 - book_pressure * 0.60, 0.35, 1.0)
            ...
            if now < sell_pause_until and inventory <= target_inv + pause_band:
                sell_size = 0.0
            if book_pressure > 0.78 and not needs_rebalance:
                buy_size = 0.0
                sell_size = 0.0
            quote_ready = now - last_quote_at >= min_quote_gap(
                ...
                order_count,
                churn_pressure,
                gap_multiplier=float(control["gap_mult"]) * (1.0 + book_pressure * 0.85),
            )
            if now - last_order_sync_at >= max(args.interval * 3.0, 0.75) and (last_bid > 0.0 or last_ask > 0.0):
                live_quotes = open_quote_prices(args.url, args.user)
                if live_quotes is not None:
                    last_bid, last_ask = live_quotes
