...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        weak_edge = (adverse > 0.65) or (regime == "shock")

        # ── Opportunistic Tightening ──
        # If stagnant and market is narrow, join the best bid/ask to get filled
        if not weak_edge and controller.target_spread_bps < market_spread_bps * 1.5:
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            if weak_edge:
                # Weak edge: quote smaller or hold. Only provide liquidity to reduce inventory.
                b_size = min(order_size * 0.5, args.max_pos - inventory) if inventory < 0 else 0.0
                s_size = min(order_size * 0.5, args.max_pos + inventory) if inventory > 0 else 0.0
            else:
