...
def quote_stale_after(
    ...
    if adverse > 0.75:
        seconds *= clamp(1.0 - (adverse - 0.75) * 0.85, 0.55, 1.0)
    if loss_pressure > 0.0:
        seconds *= clamp(1.0 - loss_pressure * 0.35, 0.70, 1.0)
    if churn_pressure > 0.0:
        seconds *= 1.0 + churn_pressure * 0.65
    return clamp(seconds, args.interval * 4.0, 75.0)
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            ...
            quote_stale = last_quote_at > 0.0 and now - last_quote_at >= quote_stale_after(
                args,
                signals,
                adverse,
                churn_pressure,
            ...
            refresh_bid = buy_size >= 1.0 and (
                ...
                or (quote_ready and (bid_price_moved or regime != last_regime or refresh_for_risk or quote_stale))
            )
            refresh_ask = sell_size >= 1.0 and (
                ((first_quote or quote_ready) and (ask_missing or ask_unsafe))
                or (quote_ready and (ask_price_moved or regime != last_regime or refresh_for_risk or quote_stale))
            )
