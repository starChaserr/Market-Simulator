...
def inventory_pain_pressure(
    account: dict[str, Any],
    mid: float,
    initial_cash: float,
    max_pos: float,
) -> float:
    inventory = float(account.get("inventory", 0.0) or 0.0)
    if abs(inventory) <= 1e-9:
        return 0.0

    avg_entry = float(account.get("avg_entry_price", 0.0) or 0.0)
    unrealized = float(account.get("unrealized_pnl", 0.0) or 0.0)
    exposure = min(abs(inventory) / max(max_pos, 1.0), 1.0)
    adverse_move = 0.0
    if avg_entry > 0.0:
        if inventory > 0.0:
            adverse_move = max(0.0, (avg_entry - mid) / avg_entry)
        else:
            adverse_move = max(0.0, (mid - avg_entry) / avg_entry)

    pnl_pressure = clamp(max(-unrealized, 0.0) / max(initial_cash * 0.0012, 1.0), 0.0, 1.0)
    move_pressure = clamp(adverse_move / 0.0030, 0.0, 1.0)
    return clamp(max(pnl_pressure, move_pressure) * (0.40 + 0.60 * exposure), 0.0, 1.0)


def pain_adjusted_target_inventory(target_inventory: float, inventory: float, pain_pressure: float) -> float:
    if pain_pressure <= 0.0 or inventory * target_inventory <= 0.0:
        return target_inventory
    return target_inventory * clamp(1.0 - pain_pressure * 0.75, 0.20, 1.0)


def inventory_pain_blocked_side(
    account: dict[str, Any],
    signals: dict[str, float | str],
    flow: float,
    pain_pressure: float,
) -> str | None:
    if pain_pressure < 0.18:
        return None
    ...
    support = (
        0.50 * clamp(flow, -1.0, 1.0)
        + 0.30 * clamp(trend / 0.004, -1.0, 1.0)
        + 0.20 * clamp(short_momentum / 0.003, -1.0, 1.0)
    )
    position_sign = 1.0 if inventory > 0.0 else -1.0
    if pain_pressure < 0.35 and position_sign * support > 0.10:
        return None
    return "buy" if inventory > 0.0 else "sell"
    ...
    try:
        while True:
            ...
            if str(signals["regime"]) in {"volatile", "shock"}:
                max_pos *= 0.62
            inventory_pain = inventory_pain_pressure(account, mid, initial_cash, max_pos)
            loss_pressure = max(loss_pressure, inventory_pain * 0.85)
            churn_pressure = max(account_churn_pressure(account), loss_pressure * 0.65)
            if loss_pressure > 0.0:
                max_pos *= clamp(1.0 - loss_pressure * 0.42, 0.48, 1.0)
            if inventory_pain > 0.0:
                max_pos *= clamp(1.0 - inventory_pain * 0.30, 0.58, 1.0)
            ...
            inventory_pain = inventory_pain_pressure(account, mid, initial_cash, max_pos)
            loss_pressure = max(account_loss_pressure(account, initial_cash), inventory_pain * 0.85)
            churn_pressure = max(account_churn_pressure(account), loss_pressure * 0.65)
            ...
            target_inv = pain_adjusted_target_inventory(target_inv, inventory, inventory_pain)
            ...
            blocked_side = inventory_pain_blocked_side(account, signals, flow, inventory_pain) or same_side_fill_lock(
                fill_tracker,
                adverse,
                inventory,
                max_pos,
