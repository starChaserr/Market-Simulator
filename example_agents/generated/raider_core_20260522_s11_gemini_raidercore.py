...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        now = time.time()
        if now - self.last_adjustment_at < 1.0: return # Faster tuning cycle (1.0s)
        
        equity = float(account.get("equity", 0))
        self.pnl_history.append(equity)
        
        # 1. Proportional Spread Tuning
        # If adverse selection is high, widen quickly and significantly.
        if adverse > 0.80:
            self.target_spread_bps = min(150.0, self.target_spread_bps * 1.4 + 5.0)
            logging.warning("CRITICAL ADVERSE: Widen to %.1f bps", self.target_spread_bps)
        elif adverse > 0.60:
            step = (adverse - 0.60) * 15.0 
            self.target_spread_bps += step
        elif len(self.pnl_history) == self.pnl_history.maxlen:
            pnl_diff = self.pnl_history[-1] - self.pnl_history[0]
            if pnl_diff < -50.0: # Losing money: defensive widening
                self.target_spread_bps += 2.0
            elif abs(pnl_diff) < 20.0: # Stagnant: opportunistic tightening
                tighten_step = 0.4 if abs(pnl_diff) < 5.0 else 0.2
                self.target_spread_bps = max(1.5, self.target_spread_bps - tighten_step)

        # Market Awareness: Stay at least near market spread
        if adverse > 0.5:
            self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 1.1)
        else:
            self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 0.9)
        
        # Elastic Recovery: Snap back if we are way wider than the market and adverse is low
        if self.target_spread_bps > market_spread_bps * 3.0 and adverse < 0.25:
            self.target_spread_bps = self.target_spread_bps * 0.75 + market_spread_bps * 0.25

        self.target_spread_bps = clamp(self.target_spread_bps, 1.5, 150.0)

        # 2. Skew Tuning (with higher intensity at extremes)
        inventory = abs(float(account.get("inventory", 0)))
        if inventory > 400:
            self.skew_intensity = min(15.0, self.skew_intensity + 1.2)
        elif inventory > 200:
            self.skew_intensity = min(10.0, self.skew_intensity + 0.5)
        elif inventory < 50:
            self.skew_intensity = max(2.0, self.skew_intensity - 0.4)

        # 3. Size Tuning
        if adverse > 0.85 or fill_rate > 4.0:
            self.order_size_mult = max(0.1, self.order_size_mult * 0.6)
        elif adverse > 0.70 or fill_rate > 2.0:
            self.order_size_mult = max(0.2, self.order_size_mult - 0.2)
        elif adverse < 0.40 and fill_rate < 0.4:
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        # If stagnant, market is narrow, and adverse selection is low, join the best bid/ask
        if controller.target_spread_bps < market_spread_bps * 1.5 and adverse < 0.35:
            if abs(inventory) < args.max_pos * 0.15:
        ...
        # "Fairness" Guardrail: Respect the market's quoting speed and respond to fills
        base_throttle = {"shock": 4.0, "volatile": 2.0, "calm": 0.5}.get(regime, 1.0)
        # Anti-sweep: wait longer if we just got filled
        throttle_gap = base_throttle * (2.0 if inventory_changed else 1.0)
        
        move_threshold = width * 0.15
        ...
        # 1. IMMEDIATE CANCELLATION for safety if price moved through us
        if toxic_quote:
            cancel_open_orders(args.url, args.user)
            last_bid = last_ask = 0.0
            logging.warning("TOXIC QUOTE: Immediate cancellation")

        # 2. STRATEGIC RE-QUOTING
        if (price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            if not toxic_quote: cancel_open_orders(args.url, args.user)
            
            # Size adjusted by controller mult and regime
            order_size = args.order_size * controller.order_size_mult
            if regime == "shock": order_size *= 0.3
            elif regime == "volatile": order_size *= 0.6
            
            # Risk Overlay: don't quote if too close to max_pos or adverse selection is too high
            b_size = min(order_size, args.max_pos - inventory)
            s_size = min(order_size, args.max_pos + inventory)
            
            # Asymmetric Side Disabling: stop quoting on the side that increases one-sided risk
            if adverse > 0.75:
                buys = tracker._fills.count("buy")
                fills_n = len(tracker._fills)
                if buys / fills_n > 0.7: # Being hit on the sell side (we are buying)
                    b_size = 0.0 # Stop buying
                    logging.warning("ASYMMETRIC RISK: Disabling BUY side")
                elif (fills_n - buys) / fills_n > 0.7: # Being hit on the buy side (we are selling)
                    s_size = 0.0 # Stop selling
                    logging.warning("ASYMMETRIC RISK: Disabling SELL side")
