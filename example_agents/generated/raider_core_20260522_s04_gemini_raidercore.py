...
class FillTracker:
    ...
    def adverse_score(self) -> float:
        n = len(self._fills)
        if n < 5: return 0.0
        buys = sum(1 for _, side in self._fills if side == "buy")
        return max(buys, n - buys) / n

    def fill_rate(self, window_sec: float = 5.0) -> float:
        if not self._fills: return 0.0
        now = time.time()
        recent = [t for t, _ in self._fills if now - t <= window_sec]
...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        ...
        # If adverse selection is high or we are churning too much, widen quickly.
        if adverse > 0.60 or fill_rate > 2.0:
            # Combined signal for widening
            adv_step = max(0, adverse - 0.60) * 15.0
            fill_step = max(0, fill_rate - 2.0) * 2.0
            self.target_spread_bps += (adv_step + fill_step)
        elif adverse < 0.35:
            # Tighten if equity is stagnant or we are just starting and have no fills
            pnl_diff = self.pnl_history[-1] - self.pnl_history[0] if len(self.pnl_history) > 1 else 0.0
            if abs(pnl_diff) < 15.0:
                # Tighten more aggressively in short/stagnant sessions
        ...
        # 3. Size Tuning - More aggressive size reduction on toxicity
        ...
        logging.info("AUTOTUNE: Spread %.1f bps (Mkt %.1f) | Skew %.1f | SizeMult %.2f | Adv %.2f | Rate %.1f", 
                     self.target_spread_bps, market_spread_bps, self.skew_intensity, self.order_size_mult, adverse, fill_rate)
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        # Be more proactive to get filled in safe markets or when our spread is competitive
        if controller.target_spread_bps < market_spread_bps * 1.3 or adverse < 0.30:
            # Asymmetric joining based on inventory room
        ...
        # ── Protection Mode ──
        # If we have lost more than 1% of starting cash in this session, widen spread by 50%
        starting_cash = args.starting_cash
        current_equity = float(account.get("equity", starting_cash))
        drawdown_pct = (starting_cash - current_equity) / starting_cash
        if drawdown_pct > 0.01:
            protection_mult = 1.0 + (drawdown_pct - 0.01) * 20.0
            width *= clamp(protection_mult, 1.0, 3.0)
            target_bid = round(min(fair - width/2 - skew, best_ask - args.tick), 4)
            target_ask = round(max(fair + width/2 - skew, best_bid + args.tick), 4)
