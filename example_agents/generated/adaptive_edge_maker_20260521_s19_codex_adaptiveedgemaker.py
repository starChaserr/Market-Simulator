...
def opportunity_guarded_sizes(
    ...
    if exposure_ratio < 0.06 and abs(target_gap) <= rebalance_band:
        if pressure <= 0.05 and regime in {"calm", "mixed"}:
            passive_scale = 0.22 if regime == "calm" else 0.16
            return buy_size * passive_scale, sell_size * passive_scale
        return 0.0, 0.0
...
def actionable_market_data(state: dict[str, Any], args: argparse.Namespace) -> tuple[bool, str]:
    ...
    if isinstance(book, dict):
        bids = book.get("bids", [])
        asks = book.get("asks", [])
        if bids and asks and (top_volume(bids, 1) <= 0.0 or top_volume(asks, 1) <= 0.0):
            return False, "empty_top_level"
