...
class FillTracker:
    ...
    def adverse_score(self) -> float:
        n = len(self._fills)
        if n < 5: return 0.0
        buys = sum(1 for _, s in self._fills if s == "buy")
        return max(buys, n - buys) / n

    def fill_rate(self, window_sec: float = 60.0) -> float:
        """Heuristic fill rate: fills per second in the last window_sec."""
        if not self._fills: return 0.0
        now = time.time()
        recent = sum(1 for t, _ in self._fills if now - t <= window_sec)
        return recent / window_sec
...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float, last_fill_time: float):
        now = time.time()
        ...
        if adverse > 0.65:
            step = (adverse - 0.60) * 8.0 # Proportional step
            self.target_spread_bps += step
        
        # Tighten more aggressively if very stagnant (no fills)
        time_since_fill = now - last_fill_time if last_fill_time > 0 else (now - self.start_time)
        if adverse < 0.40 and time_since_fill > 20.0:
            self.target_spread_bps = max(1.0, self.target_spread_bps - 0.2)

        # Market Awareness: Don't be too narrow if the market is giving us more edge
        # But if we are trying to get filled, we might need to be inside market spread.
        self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 0.8)
        
        # Regime-based minimum spread floor
        min_spread = {"shock": 10.0, "volatile": 4.0, "calm": 1.0}.get(regime, 2.0)
        self.target_spread_bps = max(self.target_spread_bps, min_spread)

        # Elastic Recovery: If we are way wider than the market and adverse is low, snap back
        if self.target_spread_bps > market_spread_bps * 3.0 and adverse < 0.35:
        ...
        if inventory > 400:
            ...
            self.skew_intensity = min(10.0, self.skew_intensity + 0.4)
        elif inventory < 50:
            self.skew_intensity = max(2.0, self.skew_intensity - 0.2)

        # 3. Size Tuning
        if adverse > 0.80 or fill_rate > 5.0:
            self.order_size_mult = max(0.1, self.order_size_mult - 0.2)
        elif adverse < 0.35 and fill_rate < 0.1 and time_since_fill > 40.0:
            self.order_size_mult = min(3.0, self.order_size_mult + 0.1)

        self.last_adjustment_at = now
        logging.info("AUTOTUNE: Spread %.1f bps (Mkt %.1f) | Skew %.1f | SizeMult %.2f | Idle %.1fs", 
                     self.target_spread_bps, market_spread_bps, self.skew_intensity, self.order_size_mult, time_since_fill)
...
def trade_loop(args: argparse.Namespace) -> int:
    logging.info("Deploying Autonomous RaiderCore v6.1 (The Balanced Self-Tuner) as %s", args.user)
    ...
    while True:
        ...
        controller.tune(account, adverse, fill_rate, regime, market_spread_bps, tracker.last_fill_time)
        fund = float(state.get("fundamental_price", mid))
        micro = microprice(state)
        
        # Fair price calculation: Adaptive blending based on regime
        if regime == "shock":
            ...
            fair = fund * 0.3 + micro * 0.3 + mid * 0.4
