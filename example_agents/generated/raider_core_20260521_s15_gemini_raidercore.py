...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        ...
        if adverse > 0.65:
            step = (adverse - 0.65) * 12.0 # Proportional step, slightly faster widening
        ...
        # Reduced from 0.9 to 0.75 to be more competitive when needed
        self.target_spread_bps = max(self.target_spread_bps, market_spread_bps * 0.75)
        
        # Elastic Recovery: If we are way wider than the market and adverse is low, snap back
        if self.target_spread_bps > market_spread_bps * 2.0 and adverse < 0.30:
        ...
        self.target_spread_bps = clamp(self.target_spread_bps, 1.1, 80.0)

        # 2. Skew Tuning (with Hysteresis)
        inventory = abs(float(account.get("inventory", 0)))
        if inventory > 250:
            self.skew_intensity = min(12.0, self.skew_intensity + 1.0)
        elif inventory < 50:
            self.skew_intensity = max(1.5, self.skew_intensity - 0.4)

        # 3. Size Tuning: Scale down if adverse or high churn, scale up only if safe
        if adverse > 0.70 or fill_rate > 2.5:
            self.order_size_mult = max(0.15, self.order_size_mult - 0.2)
        elif adverse < 0.40 and fill_rate < 0.4:
            # Check if stagnant or profitable
            pnl_diff = self.pnl_history[-1] - self.pnl_history[0] if len(self.pnl_history) > 1 else 0
            if pnl_diff >= -10.0:
                self.order_size_mult = min(2.2, self.order_size_mult + 0.08)
...
def compute_regime(state: dict[str, Any]) -> str:
    ...
    # Simple trend detection from history
    history = state.get("history", [])
    trend = 0.0
    if len(history) >= 5:
        last_mids = [float(h.get("mid", mid)) for h in history[-5:]]
        trend = (last_mids[-1] - last_mids[0]) / (last_mids[0] or mid)
    
    stress = max(disloc, vol * 1.3, abs(trend) * 2.0)
    
    if stress > 0.012: return "shock"
    if stress > 0.006: return "volatile"
    if abs(trend) > 0.0025: return "trend"
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        weights = {
            "shock": (0.65, 0.05, 0.30),    # fund, micro, mid
            "volatile": (0.50, 0.15, 0.35),
            "trend": (0.35, 0.25, 0.40),
            "calm": (0.10, 0.40, 0.50),
            "drift": (0.25, 0.30, 0.45)
        }
        fw, mw, midw = weights.get(regime, (0.3, 0.3, 0.4))
        fair = fund * fw + micro * mw + mid * midw
        ...
        inv_ratio = clamp(inventory / (args.max_pos or 1.0), -1.2, 1.2)
        # Use a more progressive skew: quadratic near zero, then linear
        skew_mult = (abs(inv_ratio) ** 1.2) * math.copysign(1.0, inv_ratio)
        skew = skew_mult * width * controller.skew_intensity
        ...
        if controller.target_spread_bps < market_spread_bps * 1.5:
            if abs(inventory) < args.max_pos * 0.15: # Wider band for tightening
        ...
        throttle_gap = {"shock": 4.0, "volatile": 2.0, "trend": 1.2, "calm": 0.6}.get(regime, 1.0)
        
        # High fill rate protection
        if fill_rate > 4.0:
            throttle_gap *= 2.0
            if int(now) % 2 == 0:
                logging.warning("CHURN PROTECTION: Increasing throttle gap due to high fill rate %.1f", fill_rate)

        move_threshold = max(args.tick * 2, width * 0.2)
