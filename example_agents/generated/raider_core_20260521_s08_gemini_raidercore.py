...
class ParametricController:
    ...
    def __init__(self):
        ...
        self.effective_size_mult = 1.0 # Combined mult
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        now = time.time()
        if now - self.last_adjustment_at < 1.0: return # Faster tuning cycle
        ...
        if adverse > 0.60:
            step = (adverse - 0.60) * 15.0 # More aggressive widening
            self.target_spread_bps += step
        elif adverse < 0.35:
            # Tighten if no adverse selection and stagnant
            history_len = len(self.pnl_history)
            if history_len >= 10: # Reduced requirement from 40 to 10
                pnl_diff = self.pnl_history[-1] - self.pnl_history[0]
                if abs(pnl_diff) < 10.0:
                    tighten_step = 0.8 if history_len < 20 else 0.3
                    self.target_spread_bps = max(1.0, self.target_spread_bps - tighten_step)

        # Market Awareness: Respect market spread but allow being tighter in calm regimes if stagnant
        mkt_floor = market_spread_bps * (0.95 if regime in ["shock", "volatile"] else 0.75)
        self.target_spread_bps = max(self.target_spread_bps, mkt_floor)
        
        # Elastic Recovery: If we are way wider than the market and adverse is low, snap back
        if self.target_spread_bps > market_spread_bps * 2.5 and adverse < 0.30:
        ...
        self.target_spread_bps = clamp(self.target_spread_bps, 1.0, 100.0)
        ...
        if inventory > 250:
            ...
            self.skew_intensity = max(2.0, self.skew_intensity - 0.5)

        # 3. Size Tuning
        base_size_mult = {"shock": 0.3, "volatile": 0.6, "calm": 1.2}.get(regime, 1.0)
        if adverse > 0.70 or fill_rate > 2.0:
            self.order_size_mult = max(0.2, self.order_size_mult - 0.2)
        elif adverse < 0.40 and fill_rate < 0.2:
            self.order_size_mult = min(2.0, self.order_size_mult + 0.1)
        
        self.effective_size_mult = self.order_size_mult * base_size_mult

        self.last_adjustment_at = now
        logging.info("AUTOTUNE: Spread %.1f bps (Mkt %.1f) | Skew %.1f | SizeMult %.2f (Eff %.2f) | Regime %s", 
                     self.target_spread_bps, market_spread_bps, self.skew_intensity, self.order_size_mult, self.effective_size_mult, regime)
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        # Book Imbalance for Fair Price
        bids = state.get("order_book", {}).get("bids", [])
        asks = state.get("order_book", {}).get("asks", [])
        bv = sum(float(lv.get("quantity", 0)) for lv in bids[:3]) 
        av = sum(float(lv.get("quantity", 0)) for lv in asks[:3])
        imbalance = (bv - av) / (bv + av + 1e-9)

        # Fair competition: Blend signals rather than exploit EMAs
        if regime == "calm":
            fair = fund * 0.15 + micro * 0.45 + mid * 0.4
        else:
            fair = fund * 0.55 + micro * 0.15 + mid * 0.3
            
        # Nudge fair price by imbalance (up to 0.5 bps)
        fair += mid * imbalance * 0.00005 

        # ── Dynamic Sizing & Width ──────────────────────────────────────
        width = mid * (controller.target_spread_bps / 10000.0)
        
        vol = float(state.get("volatility", 0.0))
        width = max(width, mid * vol * 0.6) # Slightly more vol protection
        
        # "Weak Edge" detection: survive in low-edge environments
        edge = abs(fund / mid - 1.0)
        if edge < 0.00015 and market_spread_bps < 1.5:
            width *= 1.3
            weak_edge_mult = 0.6
        else:
            weak_edge_mult = 1.0
        ...
        if (toxic_quote or price_moved or inventory_changed) and (now - last_quote_at >= throttle_gap):
            ...
            # Size adjusted by controller mult and weak edge logic
            order_size = args.order_size * controller.effective_size_mult * weak_edge_mult
