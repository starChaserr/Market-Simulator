...
def opportunity_score(
    ...
    passive_spread_edge = max(0.0, displayed_spread * 0.5 - maker_fee)
...
def runtime_tick(args: argparse.Namespace, state: dict[str, Any]) -> float:
    try:
        live_tick = float(state.get("tick", args.tick) or args.tick)
    except (TypeError, ValueError):
        live_tick = args.tick
    return max(float(args.tick), live_tick, 0.0001)


def floor_to_tick(price: float, tick: float) -> float:
    return math.floor((price + tick * 1e-6) / tick) * tick


def ceil_to_tick(price: float, tick: float) -> float:
    return math.ceil((price - tick * 1e-6) / tick) * tick


def stale_quote_feed(args: argparse.Namespace, state: dict[str, Any], signals: dict[str, float | str]) -> bool:
    ...
    return age > max(age_limit, args.interval * 12.0)


def edge_floor_quotes(
    bid: float,
    ask: float,
    fair: float,
    mid: float,
    ...
    edge_distance = max(tick, mid * min_edge)
    bid_limit = floor_to_tick(best_ask - tick, tick)
    ask_limit = ceil_to_tick(best_bid + tick, tick)
    bid = floor_to_tick(min(bid, fair - edge_distance, bid_limit), tick)
    ask = ceil_to_tick(max(ask, fair + edge_distance, ask_limit), tick)
    ...
    try:
        while True:
            ...
            if state.get("mid_price") is None or state.get("best_bid") is None or state.get("best_ask") is None:
                ...
                continue
            active_tick = runtime_tick(args, state)
            runtime_args = argparse.Namespace(**{**vars(args), "tick": active_tick})
            ...
            if stale_quote_feed(runtime_args, state, signals):
                cancel_open_orders(args.url, args.user)
                last_bid = 0.0
                last_ask = 0.0
                time.sleep(args.interval)
                continue
            width = quote_width(state, runtime_args, signals)
            inventory = float(account.get("inventory", 0.0))
            ...
            size_args = argparse.Namespace(**{**vars(runtime_args), "max_pos": max_pos, "order_size": trading_size})
            order_ratio = order_count / max(float(args.max_orders), 1.0)
            ...
            best_bid = float(state["best_bid"])
            best_ask = float(state["best_ask"])
            raw_bid = min(fair - width / 2 - inventory_skew, best_ask - active_tick)
            raw_ask = max(fair + width / 2 - inventory_skew, best_bid + active_tick)
            quote_edge_floor = passive_edge_threshold(
                runtime_args,
                state,
                signals,
                adverse,
            ...
            move_threshold = quote_move_threshold(runtime_args, width, signals, adverse)
            now = time.time()
