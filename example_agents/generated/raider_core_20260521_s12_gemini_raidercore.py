...
class ParametricController:
    ...
    def tune(self, account: dict[str, Any], adverse: float, fill_rate: float, regime: str, market_spread_bps: float):
        ...
        # 3. Size Tuning
        if adverse > 0.65 or fill_rate > 3.0 or regime == "shock":
            self.order_size_mult = max(0.1, self.order_size_mult - 0.2)
        elif regime == "volatile":
            self.order_size_mult = max(0.3, self.order_size_mult - 0.1)
        elif adverse < 0.45 and fill_rate < 0.3 and regime in ("calm", "drift"):
...
def trade_loop(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        # Determine if edge is too weak to quote at all
        weak_edge = adverse > 0.8 or (regime == "shock" and abs(inventory) > args.max_pos * 0.4)

        # Heartbeat log every 5 seconds
        if int(now) % 5 == 0 and now - last_quote_at > 0.5:
            logging.info("HEARTBEAT: Mid %.2f | Fair %.2f | BidDist %.2f | AskDist %.2f | Inv %.1f | WeakEdge %s",
                         mid, fair, mid - target_bid, target_ask - mid, inventory, weak_edge)
